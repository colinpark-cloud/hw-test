...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    QProcess::execute("sh", {"-c",
        "amixer -c wm8962audio sset 'Speaker' 100% on 2>/dev/null;"
        "amixer -c wm8962audio sset 'Headphone' 100% on 2>/dev/null"});
#endif
...
void MainWindow::audioPlayNext(bool playLeft) {
    ...
    m_audioProc->start("aplay", {"-D", "hw:wm8962audio,0", wav});
#endif
}
